...
def findTurkish(start, end, word):
    global tknew
    print(start,end)
    if (end - start) < 2:
        return start
    ...
    for i in range(len(word)):
            if word[i]>tknew[mid]:
                return findTurkish(mid, end, word)
            elif word[i]<tknew[mid]:
                return findTurkish(start, mid, word)
    return mid
...
print(tknew[findTurkish(0,len(tknew),l1[1])])
